...
public class ProjektRepository {
	...
	public void insertTest(Projekt i){
		...
		try(Connection conn = getConnection())
		{
			PreparedStatement stmt = conn.prepareStatement("insert into projektdetails (Projekt_TITLE,idRolle,Projekt_DESC,idProjektstatus,idProjekt) values (?,?,?,?)");
			stmt.setString(1, i.getProjekt_TITLE());
			stmt.setInt(5, i.getIdRolle());
	...
	private static Collection<Projekt> mapProjekt(ResultSet rs) throws SQLException 
	{
		...
		while(rs.next())
		{
			Projekt i = new Projekt(rs.getInt("idProjekt"),rs.getInt("idRolle"),rs.getInt("idProjektdetails"),rs.getString("Projektstatus_DESC"),rs.getString("Projekt_Title"),rs.getString("Projekt_DESC"),rs.getInt("idProjektstatus"));			
			list.add(i);
		}
		return list;
	}
